...
from src.api.schemas import MultiHorizonPredictionResponse, PredictionResponse, TrafficDataResponse
from src.database import TrafficRepository, create_tables, get_db
...
async def get_latest_traffic_data(limit: int = 10, db: Session = Depends(get_db)):
...
async def get_traffic_prediction():
    ...
    if prediction is None:
        return PredictionResponse(
            prediction=0, timestamp=datetime.utcnow(), status="no_prediction_available"
        )

    return PredictionResponse(prediction=prediction, timestamp=datetime.utcnow(), status="success")

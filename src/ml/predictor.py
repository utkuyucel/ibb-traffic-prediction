...
class MultiHorizonTrafficPredictor:
    ...
    def __init__(self):
        self.models = {
            horizon: RandomForestRegressor(n_estimators=100, random_state=42 + horizon)
            for horizon in self.HORIZONS
        }
        self.scalers = {horizon: StandardScaler() for horizon in self.HORIZONS}
        self.is_trained = dict.fromkeys(self.HORIZONS, False)
    ...
    def _prepare_enhanced_features(
        self, data: list[tuple[int, int, int]], target_horizon: int
        ...
        timestamps = [
            current_time.replace(second=0, microsecond=0) - pd.Timedelta(minutes=i)
    ...
    def train_models(self, data: list[tuple[int, int, int]]) -> dict[int, bool]:
        ...
        for horizon in self.HORIZONS:
            ...
            try:
                ...
                if features is None or len(features) < 5:
                    ml_logger.warning(
                        f"Insufficient data for {horizon}m horizon: "
        ...
        logger.info(
            f"Enhanced ML training completed - {successful_models}/{len(self.HORIZONS)} models trained"
        )

        return results

    def predict_multi_horizon(
        self, recent_data: list[tuple[int, int, int]]
    ) -> dict[int, int | None]:
        ...
        for horizon in self.HORIZONS:
            ...
            try:
                ...
                ml_logger.info(
                    f"{horizon}m prediction: {final_prediction} (raw: {raw_prediction:.2f})"
                )
    ...
    def _validate_consecutive_timestamps(
        self,
        data_with_timestamps: list[tuple[datetime, int, int, int]],
        ...
        if gaps_found:
            ...
            for gap in gaps_found:
                ml_logger.warning(
                    f"   Gap between {gap['previous_time']} and {gap['current_time']}: "
            ...
            ml_logger.warning(
                f"   Gap statistics - Average: {avg_gap:.1f}min, Maximum: {max_gap:.1f}min"
            )
            logger.warning(
                f"Training skipped: {len(gaps_found)} data gaps detected "
...
class TrafficPredictor(MultiHorizonTrafficPredictor):
    ...
    def prepare_features(self, data: list[tuple[int, int, int]]) -> np.ndarray | None:
    ...
    def predict_next_ti(self, recent_data: list[tuple[int, int, int]]) -> int | None:
    ...
    def evaluate_model(self, data: list[tuple[int, int, int]]) -> float | None:

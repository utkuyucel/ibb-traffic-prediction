...
class DataCollector:
    ...
    async def fetch_traffic_data(self) -> TrafficIndexData | None:
        ...
        try:
            ...
            if traffic_data:
                ...
                logger.info(
                    f"Traffic data fetched: TI={traffic_data.ti}, TI_An={traffic_data.ti_an}, TI_Av={traffic_data.ti_av}"
                )
            else:
            ...
            fetch_time = (datetime.utcnow() - fetch_start).total_seconds() * 1000
            data_logger.error(
                f"❌ Unexpected error during data fetch after {fetch_time:.2f}ms: {e}"
            )
            logger.error(f"Unexpected error during traffic data fetch: {e}")
            return None

    def _parse_json_response(self, json_content: str) -> TrafficIndexData | None:

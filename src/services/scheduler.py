...
from src.database import TrafficRepository, get_db
...
class SchedulerService:
    def __init__(self):
        ...
        self.prediction_results: dict[int, int | None] = {}
        self.data_count = 0
        ...
        try:
            ...
            try:
                ...
                if self.data_count % config.ML_TRIGGER_THRESHOLD == 0:
                    logger.info(
                        f"🎯 ML trigger threshold reached ({config.ML_TRIGGER_THRESHOLD} data points)"
                    )
                    ml_logger.info(
                        f"🚨 Training trigger activated - {self.data_count} total data points collected"
                    )
    ...
    async def train_and_predict(self, db: Session):
        ...
        try:
            ...
            recent_data_with_timestamps = TrafficRepository.get_data_for_prediction_with_timestamps(
                db, limit=200
            )
            if len(recent_data_with_timestamps) < 20:
                ml_logger.warning(
                    f"Insufficient data for ML training: {len(recent_data_with_timestamps)} records (minimum: 20)"
                )
            ...
            training_data = [
                (ti, ti_an, ti_av) for _, ti, ti_an, ti_av in recent_data_with_timestamps
            ]
            ...
            if successful_models > 0:
                ml_logger.info(
                    f"Proceeding with predictions using {successful_models} trained models"
                )
                ...
                logger.info(
                    f"ML cycle completed - {successful_predictions} predictions generated (cycle time: {cycle_time:.2f}s)"
                )
            else:
            ...
            cycle_time = (datetime.utcnow() - training_cycle_start).total_seconds()
            ml_logger.error(
                f"ML training/prediction cycle failed after {cycle_time:.2f} seconds: {e}"
            )
